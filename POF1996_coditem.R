...
#OUTROS_RECEBIMENTOS_1996c_rendimentoalt <- OUTROS_RECEBIMENTOS_1996c_rendimento %>%
#  group_by(ID) %>%
#  mutate(
#    RENDIMENTO_BRUTO_ALT_UC = sum(VALOR_ANUALIZADO, na.rm = TRUE),
#    RENDIMENTO_TOTAL = RENDIMENTO_BRUTO_UC + RENDIMENTO_BRUTO_ALT_UC
#  )

#OUTROS_RECEBIMENTOS_1996_agrupado <- OUTROS_RECEBIMENTOS_1996c_rendimentoalt %>% 
#  group_by(ID) %>% 
#  summarise(RENDIMENTO_BRUTO_UC = min(RENDIMENTO_BRUTO_UC),
#            RENDIMENTO_BRUTO_ALT_UC = min(RENDIMENTO_BRUTO_ALT_UC),
#            RENDIMENTO_TOTAL = min(RENDIMENTO_TOTAL),
#            TOTAL_DEDUCOES = min(TOTAL_DEDUCOES), 
#            RENDIMENTO_LIQ_UC = min(RENDIMENTO_LIQ_UC))

#View(OUTROS_RECEBIMENTOS_1996_agrupado)

#describe(OUTROS_RECEBIMENTOS_1996_agrupado$RENDIMENTO_BRUTO_UC)
#describe(OUTROS_RECEBIMENTOS_1996_agrupado$RENDIMENTO_TOTAL)
...
DESPESA_90DIAS_1996c_despesa <- DESPESA_90DIAS_1996c_rendimentopc %>%
  ...
  select(ID, COD_ITEM, TIPO_REGISTRO,TAMANHO_FAMILIA, FATOR_EXPANSAO, DESPESA_UC, 
...
saveRDS(DESPESAS_TOTAIS, "DESPESA_TOTAL_1996_ITEM.rds")
